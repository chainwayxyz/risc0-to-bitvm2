...
RUN (cd fflonk/circuits; circom --c --r1cs test_verify_for_guest.circom) && \
  sed -i 's/g++/clang++/' fflonk/circuits/test_verify_for_guest_cpp/Makefile && \
  sed -i 's/O3/O0/' fflonk/circuits/test_verify_for_guest_cpp/Makefile && \
  (cd fflonk/circuits/test_verify_for_guest_cpp; make)
...
COPY --from=builder /src/fflonk/circuits/test_verify_for_guest_cpp/test_verify_for_guest /app/test_verify_for_guest
COPY --from=builder /src/fflonk/circuits/test_verify_for_guest_cpp/test_verify_for_guest.dat /app/test_verify_for_guest.dat
